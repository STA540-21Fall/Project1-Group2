...
geographic = read.csv("data/uscounties.csv")[,c(4,7,8)]
```
...
geographic_nyc = geographic_nyc[,c(3,1,2)]
...
final_cases = cases_deaths %>% 
  mutate(peaks = case_when (date >= "2020-07-20" & date <= "2020-07-26" ~ "peak 1",
                            ...
                            date >= "2021-08-30" & date <= "2021-09-05" ~ "peak 3")) %>%
  filter(!is.na(peaks)) %>% 
  filter(fips!= "   NA") %>% 
  group_by(peaks, fips) %>% 
...
final$income_group <- cut(final$median.income,
                          breaks = c(0,40100,120400,Inf), 
                          labels = c('Low Income','Middle Income','Upper Income'))

counties <- get_urbn_map(map = "counties", sf = TRUE)
total <- merge(counties, final_new, by.y = "fips", 
                by.x = "county_fips")

#median income total

pal <- hcl.colors(3, "Inferno", rev = TRUE, alpha = 0.7)

ggplot()+
  geom_sf(data = total, mapping = aes(fill = income_group), color = "white", lwd = 0.1) +
...
#cases peak 1
ggplot()+
  geom_sf(data = final_peak1, mapping = aes(fill = cases), color = "white", lwd = 0.1) +
  theme_dark()

#unemployed rate peak 1
ggplot()+
  geom_sf(data = total, mapping = aes(fill = unemployed.rate), color = "white", 
          lwd = 0.1) +
  theme_dark()
```
